...
const ExpandButton = ({ question, answer, value, setValue, svg }) => {
    return (
        <button className="w-full py-4 px-2 border-divider border-b-2" onClick={(e) => {e.preventDefault(); setValue(!value)}}>
            <div className="flex justify-between items-start gap-8">
                ...
                <p className="text-body text-left min-w-[186px]">{question}</p>
            ...
            <p className={value? "text-small-body text-start my-4" : "hidden"}>{answer}</p>
        </button>
    )
}
