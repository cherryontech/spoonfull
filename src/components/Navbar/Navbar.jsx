...
const Navbar = ({
  ...
  useEffect(() => {
    const spoonCount = (arr) => {
      ...
      const used = checkedArray.reduce(
        (accumulator, spoon) => accumulator + spoon,
        0
      );
      ...
      const totalSpoons = spoonArray.reduce(
        (accumulator, currentSpoon) => accumulator + currentSpoon,
        0
      );
  ...
  return (
    ...
    >
      <Box sx={{ flexGrow: 1 }}>
        <AppBar
          className="xl:pl-[72px] xl:pr-20 md:pl-1 pr-6 pb-1 sm:pr-2 py-2"
          position="fixed"
          style={{ background: "#FCFEFE" }}
          elevation={0}
        >
          <Toolbar
            sx={{
          ...
          >
            ...
            <button
              className="md: w-[46px] h-[49px]"
              onClick={() => {
                setShowSpoonsModal(true);
              }}
              sx={{ pt: 8, alignContent: "center" }}
            >
              {remainingSpoons > 3 ? (
                <Badge
                  badgeContent={remainingSpoons}
                  sx={{
                    "& .MuiBadge-badge": {
                      backgroundColor: "#23A1AF",
                      color: "white",
                    },
                  }}
                >
                  <div className="pr-1.5 self-start">
                    ...
                    >
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M10.025 11.4492L3.68505 6.42088C3.46973 6.25033 3.29292 6.03615 3.16624 5.79241C3.03956 5.54868 2.96588 5.28091 2.95002 5.00667C2.93417 4.73244 2.97651 4.45796 3.07426 4.20125C3.17201 3.94455 3.32295 3.71142 3.51719 3.51719C3.71142 3.32295 3.94455 3.17201 4.20125 3.07426C4.45796 2.97651 4.73244 2.93417 5.00667 2.95002C5.28091 2.96588 5.54868 3.03956 5.79241 3.16624C6.03615 3.29292 6.25033 3.46973 6.42088 3.68505L11.4492 10.025C12.5384 9.54922 14.1992 9.48505 15.5984 10.8834C16.4442 11.7301 16.9709 12.765 17.1509 13.75C17.3259 14.7084 17.1917 15.7717 16.4817 16.4817C15.7725 17.1917 14.7084 17.3259 13.7509 17.1509C12.7659 16.9709 11.73 16.4442 10.8842 15.5975C9.48422 14.1984 9.54922 12.5384 10.025 11.4492Z"
                        fill="#0F0129"
                      />
                    </svg>
                  </div>
      ...
      >
          ...
          <List>
            {navigation.map((item) => {
              return (
                ...
                >
                  <ListItemIcon sx={{ mr: -3 }} style={{ color: "#001111" }}>
                    {" "}
                    {item.icon}{" "}
                  </ListItemIcon>
                  {item.name}
        ...
        <footer className="px-4 flex justify-end">
          ...
          {mediumScreen ? (
            <p className="pl-2 text-text1 text-body">
              SpoonFull, all rights reserved
            </p>
          ) : (
            <p className="pl-2 text-text1 text-caption">
              SpoonFull, all rights reserved
            </p>
          )}
        </footer>
